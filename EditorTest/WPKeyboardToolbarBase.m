...
//    }
    if (_imageButton == nil) {
        self.imageButton = [WPKeyboardToolbarButtonItem button];
        [_imageButton setImageName:@"icon_format_media" withColor:nil highlightColor:highlightColor];
        _imageButton.frame = CGRectMake(x, 0, width, height);
		x += _imageButton.frame.size.width;
        _imageButton.actionTag = @"image";
        _imageButton.accessibilityIdentifier = @"image";
        _imageButton.actionName = NSLocalizedString(@"image", @"Image HTML formatting in the Post Editor.");
        _imageButton.accessibilityLabel = NSLocalizedString(@"image", nil);
        _imageButton.exclusiveTouch = YES;
...
//        [_formatView addSubview:_quoteButton];
        [_formatView addSubview:_imageButton];
